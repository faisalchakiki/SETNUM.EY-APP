...
const managePin = () => {
  ...
  // VALIDATION PIN
  const validationPin =(event)=>{
    event.preventDefault()
    if(pinResult === profile.pin) {
      return route.push("/profile/new-pin");
    }else {
      alert('pin fail')
    }
  }
        ...
        <div className="w-[70%] rounded-[8px]">
          <section className="bg-white rounded-[8px] px-[20px] py-[30px]">
            ...
            <p className="w-[80%] my-2">
              Enter your current 6 digits SETMON.EY PIN below to continue to the
              next steps.
            </p>
            <form onSubmit={validationPin} className="w-[70%] mx-auto mt-[50px]">
